...
export class UserController {
  ...
  static loginAuthenticate = async (
    req: Request,
    res: Response,
    next: NextFunction
  ) => {
  ...
  static getHomepage = async (
    ...
    if (empId) {
      ...
      for (var j = 0; j < allLeaves.length; j++) {
        ...
        if (
          allLeaves[j].status == "Pending" &&
          allLeaves[j].leaveType == "Casual Leave"
        )
          casualPending += 1;

        if (
          allLeaves[j].status == "Pending" &&
          allLeaves[j].leaveType == "Sick Leave"
        )
          sickPending += 1;

        if (
          allLeaves[j].status == "Pending" &&
          allLeaves[j].leaveType == "Earned leave"
        )
          earnedPending += 1;

        if (
          allLeaves[j].status == "Pending" &&
          allLeaves[j].leaveType == "Loss Of Pay"
        )
          lopPending += 1;

        //counting availed leaves
        if (
          allLeaves[j].status == "Approved" &&
          allLeaves[j].leaveType == "Casual Leave"
        )
          casualAvailed += 1; //count of availed casualleave

        if (
          allLeaves[j].status == "Approved" &&
          allLeaves[j].leaveType == "Sick Leave"
        )
          sickAvailed += 1; //count of availed sickleave
